...
class KNNModel:
    ...
    def predict(self, X):
        ...
        y_pred = np.zeros(n)
        for i in range(n):
            dists = np.linalg.norm(np.array(self.X, dtype=float) - np.array(X[i], dtype=float), axis=1)
            closest_k = np.argsort(dists)[:self.k]
            y_pred[i] = np.argmax(np.bincount(self.y[closest_k]))
        return y_pred
    ...
    def fit(self, x_train, y_train):
        self.train(x_train, np.argmax(y_train, axis=1))

    def score(self, x_train, y_train):
        return self.accuracy(x_train, np.argmax(y_train, axis=1))
